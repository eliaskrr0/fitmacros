...
fun ExportScreen(
    ...
    onNavigateUp: () -> Unit,
    onDietaSelected: (Dieta) -> Unit
) {
    val dietasState by dietaViewModel.uiState.collectAsState()
    ...
    Scaffold(
        ...
        Column(
            ...
            LazyColumn(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                items(dietasState.dietas) { dieta ->
                    DietaExportItem(dieta = dieta, onClick = { onDietaSelected(dieta) })
                }
            }
        }
    }
...
@Composable
private fun DietaExportItem(dieta: Dieta, onClick: () -> Unit) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
