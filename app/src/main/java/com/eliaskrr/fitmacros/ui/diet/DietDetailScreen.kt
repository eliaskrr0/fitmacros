    ...
    Column(modifier = Modifier.padding(vertical = 8.dp)) {
        ...
        if (mealData.items.isEmpty()) {
            ...
            mealData.items.forEach { item ->
                val isSelected = selectedIds.contains(item.food.id)
                AlimentoInDietaItem(
                    item = item,
                    ...
                    onClick = {
                        if (uiState.isSelectionMode) {
                            viewModel.toggleSelection(mealType, item.food.id)
                        } else {
                            viewModel.startEditQuantity(mealType, item)
                        }
                    },
                    onLongPress = { viewModel.toggleSelection(mealType, item.food.id) }
                )
            }
        }
